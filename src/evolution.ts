...
function resolveTarget(cwd: string, target: string): string {
  ...
  const root = resolve(cwd); const absolute = resolve(root, target);
...
export async function proposeEvolution(target: string, proposedContent: string, reason: string, sourceLearning?: string, cwd = process.cwd()): Promise<EvolutionCandidate> {
  const absolute = resolveTarget(cwd, target); const baseExisted = await exists(absolute); const base = await readOptional(absolute);
...
export async function evaluateEvolution(id: string, command: string, cwd = process.cwd()): Promise<EvolutionEval> {
  ...
  const target = resolveTarget(cwd, candidate.target); const scratchDir = resolve(cwd, '.agents/evolution-eval', id); await mkdir(scratchDir, { recursive: true }); const candidateFile = resolve(scratchDir, 'candidate.txt'); await writeFile(candidateFile, candidate.proposedContent);
...
export async function approveEvolution(id: string, cwd = process.cwd()): Promise<EvolutionCandidate> {
  ...
  candidate.status = 'approved'; candidate.approvedAt = new Date().toISOString(); await writeCandidate(cwd, candidate); return candidate;
}
...
export async function applyEvolution(id: string, cwd = process.cwd()): Promise<EvolutionCandidate> {
  ...
  const target = resolveTarget(cwd, candidate.target); const current = await readOptional(target);
  ...
  candidate.status = 'applied'; candidate.appliedAt = new Date().toISOString(); await writeCandidate(cwd, candidate); return candidate;
}
...
export async function rollbackEvolution(id: string, cwd = process.cwd()): Promise<EvolutionCandidate> {
  ...
  candidate.status = 'rolled_back'; candidate.rolledBackAt = new Date().toISOString(); await writeCandidate(cwd, candidate); return candidate;
}
