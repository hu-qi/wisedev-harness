...
program.name('wisedev-harness').description('WiseDev agent harness runtime and verification CLI').version('0.4.0');
...
program.command('init').description('Initialize project harness without clobbering existing agent instructions').option('--force', 'replace the manifest; managed agent blocks remain merge-safe').action(async opts => {
...
program.command('trust').description('Trust the exact current manifest fingerprint for command hooks').action(async () => console.log(`Trusted manifest ${(await trustManifest()).manifestSha256}`));
program.command('untrust').description('Revoke local hook execution trust').action(async () => { await revokeTrust(); console.log('Manifest hook trust revoked.'); });
...
session.command('record').description('Record a redacted session event').requiredOption('--type <type>', 'note|intervention|tool_failure|tool_denied|retry').requiredOption('--message <message>').action(async opts => {
  const event = await recordSessionEvent(opts.type as SessionEventType, opts.message);
...
session.command('end').description('End session, score friction and create a learning candidate when warranted').option('--threshold <score>', 'candidate threshold', '5').action(async opts => {
  const { summary, candidate } = await endSession(process.cwd(), Number(opts.threshold));
...
program.command('recall <query...>').description('Search tracked learnings with explainable lexical ranking').option('--limit <n>', 'maximum results', '5').action(async (query, opts) => {
  const results = await recallLearnings(query.join(' '), process.cwd(), Number(opts.limit));
  for (const item of results) console.log(`${item.score}\t${item.file}\tmatched=${item.matched.join(',')}\t${item.learning.title}\n  ${item.learning.summary}`);
});
...
program.command('doctor').description('Diagnose and reconcile generated runtime adapter blocks').option('--fix', 'rewrite managed runtime blocks').action(async opts => {
  const before = await verifyHarness(); print(before);
  if (opts.fix && hasFailures(before)) { const manifest = await loadManifest(); for (const file of await applyAdapters(manifest)) console.log(`Reconciled ${file}`); process.exitCode = hasFailures(await verifyHarness()) ? 1 : 0; }
});
